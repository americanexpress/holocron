...
// FIXME: support the configuration
 ...
 */
const fetchAsset = async (assetUrl, parseAsJson, maxRetries = 3) => {
  const { protocol } = new URL(assetUrl);

  if (process.env.NODE_ENV === 'production' && protocol !== 'https:') {
    throw new Error('HTTPS must be used to load modules in production');
  }
